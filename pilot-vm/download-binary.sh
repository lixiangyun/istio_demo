...
docker cp $CONTAINER_ID:/usr/local/bin/pilot-discovery ./pilot-discovery/

CONTAINER_ID=$(docker ps | grep consul_consul_1 | awk  '{print $1}' )
docker cp $CONTAINER_ID:/bin/consul ./consul/

CONTAINER_ID=$(docker ps | grep istio-apiserver_1 | awk  '{print $1}' )
docker cp $CONTAINER_ID:/usr/local/bin/kube-apiserver ./kube-apiserver/

CONTAINER_ID=$(docker ps | grep productpage-v1-sidecar_1 | awk  '{print $1}' )
